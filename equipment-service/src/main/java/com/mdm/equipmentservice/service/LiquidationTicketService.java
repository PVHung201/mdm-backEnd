...
public interface LiquidationTicketService {
    ...
    Page<EquipmentListLiquidationDto> getAllEquipmentsForLiquidation(GetEquipmentsForLiquidationQueryParam getEquipmentsForLiquidationQueryParam,
                                                                     Pageable pageable);

    @PreAuthorize("hasAnyRole('ROLE_ADMIN','ROLE_TPVT') or hasAuthority(\"LIQUIDATION.CREATE\")")
    LiquidationTicketFullInfoDto createLiquidationTicket(CreateLiquidationTicketForm createLiquidationTicketForm, Long equipmentId,
