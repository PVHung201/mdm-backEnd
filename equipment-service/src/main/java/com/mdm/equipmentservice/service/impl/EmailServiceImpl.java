...
public class EmailServiceImpl implements EmailService {
    ...
    public EmailServiceImpl(
            ...
            MessageUtil messageUtil, UserService userService, FileStorageService fileStorageService,
...
//    @Order(Ordered.HIGHEST_PRECEDENCE)
//    @EventListener
//    public void sendEmail(TransferTicketAcceptedEvent event) throws MessagingException {
//        log.info("sending repair ticket accepted email...");
//        MimeMessage message = mailSender.createMimeMessage();
//        MimeMessageHelper helper = new MimeMessageHelper(message, true);
//        createTemplate(event, helper);
...
//    }
    ...
    public List<String> getAllEmailThatCanReceiveNotification(NotificationType notificationType) {
        ...
        List<UserDetailDto> allUserInRoles = userService.getAllUserInRoles(allRoleThatCanReceiveNotification).stream().toList();
    ...
    private void createTemplate(LiquidationTicketCreatedEvent event, MimeMessageHelper helper) throws MessagingException {
        ...
        helper.setText(String.format(
                ...
                CommonUtil.convertToCurrency(liquidationTicket.getPrice().toString()),
                creator.getName(),
    ...
    private void createTemplate(MaintenanceTicketCreatedEvent event, MimeMessageHelper helper) throws MessagingException {
        ...
        Department department = equipment.getDepartment();
        CompletableFuture<List<FileStorage>> getDocumentsAsync = CompletableFuture.supplyAsync(() -> fileStorageService.getAllFilesOfAnEntity(
        ...
        helper.setSubject(String.format("""
                [Bảo trì] Phiếu bảo trì, bảo dưỡng thiết bị [%s] đã được tạo.
                """, equipment.getName()));
        helper.setText(String.format(
                """
                        Phiếu bảo trì thiết bị: [%s] đã được tạo. Thiết bị đã được chuyển sang danh sách: [Chờ duyệt bảo dưỡng].
                        Serial: [%s]
                        Model: [%s]
                        Mã thiết bị: [%s]
                        Khoa Phòng ban: [%s]
                ...
                maintenanceTicket.getCreatorNote()
        ));
        List<FileStorage> documents = getDocumentsAsync.join();
        addAttachment(helper, documents);
    }
    ...
    private void createTemplate(MaintenanceTicketUpdatedEvent event, MimeMessageHelper helper) throws MessagingException {
        ...
        helper.setText(String.format(
                """
                        Phiếu bảo trì thiết bị: [%s] đã được cập nhật. Thiết bị đã được chuyển sang danh sách: [Đang sử dụng].
                        Serial: [%s]
                        Model: [%s]
                        Mã thiết bị: [%s]
                        Khoa Phòng ban: [%s]
    ...
    private void createTemplate(TransferTicketAcceptedEvent event, MimeMessageHelper helper) throws MessagingException {
        TransferTicket transferTicket = event.getTransferTicket();
        Department fromDepartment = transferTicket.getFromDepartment();
        Department toDepartment = transferTicket.getToDepartment();
        Equipment equipment = transferTicket.getEquipment();
        User creator = transferTicket.getCreator();
        User approver = transferTicket.getApprover();
        List<FileStorage> documents = fileStorageService.getAllFilesOfAnEntity(
                transferTicket.getClass().getSimpleName(),
                transferTicket.getId(),
                FileDescription.DOCUMENT
        ...
        helper.setTo(creator.getEmail());
        helper.setCc(getCcEmails(NotificationType.LIQUIDATION, creator.getEmail()));
        String title = transferTicket.getStatus().equals(TicketStatus.ACCEPTED) ? "được chấp nhận" : "bị từ chối";
        String subject = transferTicket.getStatus().equals(TicketStatus.ACCEPTED) ? "Chấp nhận" : "Từ chối";
        helper.setSubject(String.format("""
                [Điều chuyển][%s] Phiếu điều chuyển thiết bị [%s] %s.
                """, subject, equipment.getName(), title));
        helper.setText(String.format(
                """
                        Phiếu điều chuyển thiết bị: [%s] đã %s.
                        Thiết bị đã được chuyển sang khoa phòng [%s]
                        Serial: [%s]
                        Model: [%s]
                        Mã thiết bị: [%s]
                        Khoa Phòng ban điều chuyển: [%s]
                        Ngày điều chuyển: [%s]
                        Người tạo phiếu: [%s]
                        Ngày tạo phiếu: [%s]
                        Ghi chú của người tạo phiếu: [%s]
                ...
                transferTicket.getCreatorNote(),
                approver.getName(),
                transferTicket.getApprovalDate().format(DATE_TIME_FORMAT),
                transferTicket.getApproverNote()
        ));
        addAttachment(helper, documents);
    }
    ...
    private void createTemplate(MaintenanceTicketAcceptedEvent event, MimeMessageHelper helper) throws MessagingException {
        ...
        String status = maintenanceTicket.getStatus().equals(TicketStatus.ACCEPTED) ? "Đang bảo dưỡng" : "Đang sử dụng";
        helper.setSubject(String.format("""
    ...
    private void createTemplate(InspectionTicketCreatedEvent event, MimeMessageHelper helper) throws MessagingException {
        ...
        helper.setText(String.format(
                ...
                inspectionTicket.getCreatorNote()
        ));
        addAttachment(helper, documents);
    }
    ...
    private void createTemplate(InspectionTicketUpdatedEvent event, MimeMessageHelper helper) throws MessagingException {
        ...
        helper.setSubject(String.format("""
                ...
                """, equipment.getName()));
        String evaluationStatusForEmail = getInspectionEvaluationStatusForEmail(inspectionTicket.getEvaluationStatus());
        String equipmentStatusForEmail = getEquipmentStatusFromEvaluationStatusForEmail(inspectionTicket.getEvaluationStatus());
        helper.setText(String.format(
                """
                        Phiếu kiểm định thiết bị: [%s] đã được cập nhật.
                        Đánh giá tình trạng thiết bị: [%s]
                        Thiết bị đã được chuyển sang danh sách: [%s].
                        Serial: [%s]
                        Model: [%s]
                        Mã thiết bị: [%s]
                        Khoa phòng ban: [%s]
                        ...
                        """,
                equipment.getName(),
                evaluationStatusForEmail,
                equipmentStatusForEmail,
                equipment.getSerial(),
                equipment.getModel(),
                equipment.getHashCode(),
                department.getName(),
    ...
    private String getEquipmentStatusFromEvaluationStatusForEmail(InspectionEvaluationStatus evaluationStatus) {
        return switch (evaluationStatus) {
            case GOOD -> "Đang sử dụng";
            case BAD_CAN_REPAIR -> "Hỏng";
            case BAD_CANNOT_REPAIR -> "Ngừng sử dụng";
        };
    }

    private String getInspectionEvaluationStatusForEmail(InspectionEvaluationStatus status) {
        return switch (status) {
            case GOOD -> "Tốt";
            case BAD_CAN_REPAIR -> "Có thể sửa chữa";
            case BAD_CANNOT_REPAIR -> "Không thể khắc phục";
        };
    }
    ...
    private void createTemplate(InspectionTicketAcceptedEvent event, MimeMessageHelper helper) throws MessagingException {
        ...
        String status = inspectionTicket.getStatus().equals(TicketStatus.ACCEPTED) ? "Đang kiểm định" : "Đang sử dụng";
        helper.setSubject(String.format("""
        ...
        helper.setText(String.format(
                ...
                inspectionTicket.getApprovalDate().format(DATE_TIME_FORMAT),
                inspectionTicket.getApproverNote()
        ));
        addAttachment(helper, documents);
    }

    public void addAttachment(MimeMessageHelper helper, List<FileStorage> fileStorages) {
