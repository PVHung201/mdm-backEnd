...
//        return maintenanceTicketMapper.toFullInfoDto(savedMaintenanceTicket);
    ...
    public MaintenanceTicketFullInfoDto acceptMaintenanceTicket(
        ...
        MaintenanceTicket maintenanceTicket = maintenanceRepository.findById(maintenanceId)
                .orElseThrow(() -> new ResourceNotFoundException(messageUtil.getMessage("maintenanceNotFound")));
        Equipment equipment = maintenanceTicket.getEquipment();
        if (!equipment.getId().equals(equipmentId)) {
            throw new ResourceNotFoundException(messageUtil.getMessage("equipmentNotFound"));
        }
        ...
        maintenanceRepository.save(maintenanceTicket);
        applicationEventPublisher.publishEvent(new MaintenanceTicketAcceptedEvent(maintenanceTicket));
//        return maintenanceTicketMapper.toFullInfoDto(maintenanceTicket);
