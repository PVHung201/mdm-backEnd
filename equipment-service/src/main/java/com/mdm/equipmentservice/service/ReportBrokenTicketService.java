...
public interface ReportBrokenTicketService {
    ...
    Page<EquipmentListReportBrokenDto> getAllEquipmentsForReportBroken(GetEquipmentsForReportBrokenQueryParam getEquipmentsForReportBrokenQueryParam,
                                                                       Pageable pageable);

    @PreAuthorize("hasAnyRole('ROLE_ADMIN','ROLE_TPVT') or hasAuthority(\"REPORT_BROKEN.CREATE\")")
    ReportBrokenTicketFullInfoDto createReportBrokenTicket(CreateReportBrokenTicketForm createReportBrokenTicketForm, Long equipmentId,
