...
public class UserServiceImpl implements UserService {
    ...
    @Autowired
    public UserServiceImpl(UserRepository userRepository, BCryptPasswordEncoder bCryptPasswordEncoder, ForgotPasswordMapper forgotPasswordMapper,
                           UniqueValidationUtil uniqueValidationUtil, MessageUtil messageUtil, Configuration configuration, Keycloak keycloak,
                           UserMapper userMapper,
                           KeycloakUserRepository keycloakUserRepository, KeycloakUserService keycloakUserService,
    ...
    private void checkIfIsTryingToUpdateUsername(UpsertUserForm upsertUserForm, User userToBeUpdated) {
        if (isTryingToEditUsername(upsertUserForm, userToBeUpdated) && !this.realmResource.toRepresentation().isEditUsernameAllowed()) {
    ...
    @Override
    public Set<UserDetailDto> getAllUserInRoles(List<String> roles) {
        if (roles == null || roles.isEmpty()) {
            return new HashSet<>();
        }
        List<User> users = userRepository.findByRoleNameInIgnoreCase(roles);
        return userMapper.toUserDetailDtoSet(new HashSet<>(users));
    }
