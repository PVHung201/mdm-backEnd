...
public abstract class IdToEntityMapper {
    ...
    public Set<Department> toListDepartment(Set<Long> departmentIds) {
        if (departmentIds == null || departmentIds.isEmpty()) {
            return null;
        }
        Set<Long> blankFilteredDepartmentIds = departmentIds.stream().filter(ObjectUtils::isEmpty).collect(Collectors.toSet());
        return new HashSet<>(departmentRepository.findAllById(blankFilteredDepartmentIds));
    }
    ...
    Set<Supply> toSupplies(Set<Long> supplyIds) {
        if (supplyIds.isEmpty())
            return null;
        return new HashSet<>(supplyRepository.findAllById(supplyIds));
    }
    ...
    Set<EquipmentSupplyUsage> toEquipmentSupplyUsages(Set<Long> equipmentSupplyUsageIds) {
        if (equipmentSupplyUsageIds == null || equipmentSupplyUsageIds.isEmpty())
            return null;
        return new HashSet<>(equipmentSupplyUsageRepository.findAllById(equipmentSupplyUsageIds));
    }
    ...
    Set<Supplier> toSuppliers(Set<Long> supplierIds) {
        if (supplierIds.isEmpty())
            return null;
        return new HashSet<>(supplierRepository.findAllById(supplierIds));
    }

    Set<Service> serviceIdsToServices(Set<Long> serviceIds) {
        if (serviceIds == null || serviceIds.isEmpty())
            return null;
        return ObjectUtils.isNotEmpty(serviceIds) ? new HashSet<>(serviceRepository.findAllById(serviceIds)) : null;
    }
    ...
    Set<Equipment> toEquipments(Set<Long> equipmentIds) {
