...
@Mapper(
        componentModel = MappingConstants.ComponentModel.SPRING, imports = {
...
public abstract class RepairTicketMapper {
    ...
    public abstract RepairTicket createTicket(CreateRepairTicketForm createRepairTicketForm, Equipment equipment);

    @Mapping(
            target = "attachments",
            expression = "java(fileStorageService.getAllFilesOfAnEntityWithoutData(repairTicket.getClass().getSimpleName(),repairTicket.getId(),FileDescription.DOCUMENT))"
    )
